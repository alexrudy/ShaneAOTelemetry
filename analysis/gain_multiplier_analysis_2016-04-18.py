...
def generate_table(query_base, eliminate):
    ...
    query_default = query_base.filter(ShaneAOInfo.control_matrix == "controlMatrix_16x.fits")
    query_midrange = query_base.filter(ShaneAOInfo.control_matrix == "controlMatrix_16x.incgain.250Hz.fits")
    query_boosted = query_base.filter(ShaneAOInfo.control_matrix == "controlMatrix_16x.incgain.1000Hz.fits")
...
def main():
    ...
    with app.app_context():
        ...
        query = query.filter(Dataset.date.between(datetime.date(2016, 04, 18), datetime.date(2016, 04, 20)))

...
from telemetry.ext.fourieranalysis.tasks import transferfunction_plot
...
def main():
    ...
    with app.app_context():
        session = app.session
        path = "transferfunction/{0}".format(opt.kind)
        ...
        g = group(transferfunction_plot.si(dataset.id, opt.kind) for dataset in query.all())
